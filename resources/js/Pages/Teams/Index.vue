  ...
  <div class="mx-auto max-w-5xl p-6 space-y-8">
    ...
    <div class="grid gap-6 md:grid-cols-2">
      ...
      <div class="space-y-3">
        ...
        <div
          v-if="!hasOwned"
          class="rounded border border-dashed p-4 text-sm text-gray-600 bg-white"
        >
          You don't own any team yet.
        </div>
        <ul v-else class="space-y-2">
          <li
            v-for="t in ownedTeams"
            :key="t.id"
          ...
          >
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                <span
                  class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-emerald-100 text-emerald-800"
                >
                  OWNER
                </span>
                <Link
                  :href="route('teams.show', t.id)"
          ...
          >
            <div class="flex items-center justify-between">
              <div>
                ...
                <div class="text-xs text-gray-500">
                  Owner:
                  <span class="font-medium">{{ t.owner?.name ?? '-' }}</span>
    ...
    <!-- 🔹 Floating Home Button (isteğe bağlı) -->
    <Link
      :href="route('dashboard')"
      class="fixed bottom-6 right-6 rounded-full bg-indigo-600 px-4 py-2 text-white shadow-lg hover:bg-indigo-700"
    >
      Home
    </Link>
  </div>
